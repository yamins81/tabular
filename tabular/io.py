...
__all__ = ['loadSV', 'loadSVcols','loadSVrecs', 'saveSV', 'loadbinary',       
           ...
           'appendHSV', 'appendcolumns', 'inferdelimiterfromname', 
...
def loadSVcols(fname, usecols=None,excludecols = None, valuefixer=None, colfixer=None, missingvalues=None,fillingvalues=None,typeinferer = None, **kwargs):
    ...
    **See Also:**
    ...
    if usecols is not None:
        ...
        if excludecols is not None:
            dontget = [i if i >= 0 else l0 + i for i in excludecols if isinstance(i,int)]
            if 'names' in metadata.keys():
    ...
    return [[preparecol(records,j,formats[j],valfix[j],colfix[j],missval[j],fillval[j],typer[j]) for j in getcols],metadata]
  
def preparecol(records,j,format,valfix,colfix,missval,fillval,typer):

    assert (typer is None or (fillval is None or missval is None)), 'If typeinferer is set for a given column then neither fillingvalues or missingvalues can be set for that column'
...
def loadSVrecs(fname, uselines = None, skiprows=0,
                linefixer=None, delimiter_regex = None, verbosity = 
                DEFAULT_VERBOSITY, **metadata):
    ...
    character, and return data as a list of lists of strings corresponding to records (rows).  Also uses and returns metadata (including column names, formats, coloring, &c.) if these items are determined during the loading process.   
...
def getmetadata(fname, inflines = 2500, linefixer=None,delimiter_regex = None, namesinheader=True, skiprows=0, verbosity = DEFAULT_VERBOSITY, comments = None,  **metadata):
    ...
    storedmetadata = readstoredmetadata(fname,skiprows=skiprows,comments=comments, metametadata=mmd, verbosity=verbosity)
    
    if storedmetadata:
        if verbosity > 7:
            print '\n\nStored metadata read from file:', storedmetadata, '\n\n' 
        for name in storedmetadata:
            if name in metadata.keys() and (storedmetadata[name] != None) and storedmetadata[name] != metadata[name]:
                if verbosity >= 4:
    ...
    if 'headerlines' not in metadata.keys():
       metadata['headerlines'] = inferheader(F,metadata=metadata,verbosity=verbosity)
       
    if namesinheader and (metadata['headerlines'] == 0 or metadata['headerlines'] is None):
    ...
    if 'dialect' not in metadata.keys():
        if 'delimiter' not in metadata.keys():
            ...
            infdia = inferdialect(fname=fname,datalines=F[metadata['headerlines']:],delimiter_regex=delimiter_regex)
            metadata['dialect'] = infdia
            printdialectinferencemessage(metadata['dialect'],verbosity,delimiter_regex)
        else:
            metadata['dialect'] = csv.Sniffer().sniff(metadata['delimiter'])
 
    processmetadata(metadata,items='dialect',comments=comments,verbosity=verbosity)

    if delimiter_infer:
        printdelimitercheckmessage(metadata['dialect'],infdia,storedmetadata,verbosity)
    ...
    processmetadata(metadata,items='names,formats,types',comments=comments,delimiter_regex=delimiter_regex,verbosity=verbosity)
...
def saveSV(fname, X, comments=None, metadata=None, printmetadict=None,
                   ...
                   quotechar='"', skipinitialspace=False, stringifier=None,
    ...
    **Parameters**
            ...
            **delimiter** :  string, optional
                    ...
                    The character to beused to separate values in each line of text, e.g. ','.  If not specified, by default, this is inferred from the file extension: if the file ends in `.csv`, the delimiter is ',', otherwise it is '\\t.'

            **linebreak** :  string, optional

                    The string separating lines of text.  By default, this is assumed to be '\\n', and can also be set to be '\\r' or '\\r\\n'.

            **metadata** :  list of strings or Boolean, optional

                    Allowed values are True, False, or any sublists of the list 
                    `['names', 'formats', 'types', 'coloring', 'dialect']`.  These keys indicate what special metadata is printed in the header.
                    ...
                    *   If `True`, this is the same as 
                        `metadata = ['coloring', 'types', 'names','dialect']`, e.g. as many types of metadata as this algorithm currently knows how to write out. 
    ...
    **See Also:**
    ...
    if metadata is None:
        ...
        if printmetadict is None:
            printmetadict = False
            if verbosity > 8:
                print 'Defaulting to not printing out the metametadata dictionary line.'
        if comments is None:
            comments = ''
            if verbosity > 8:
        ...
        if printmetadict is None:
            printmetadict = True
            if verbosity > 8:
                print 'Defaulting to printing out the metametadata dictionary line.'            
        if comments is None:
            comments = ''
            if verbosity > 8:
                print 'Defaulting empty comment string.'            
        ...
        if comments is None:
            comments = ''
        if verbosity >= 5:
            print 'Using user-specified metadata keys to contol metadata writing.'
...
def Write(X,F,dialect,order = None,stringifier=None):
    ...
    for (i,name) in enumerate(Order):
        ...
        if typename.startswith('str'):
            if any([delimiter in d or lineterminator in d for d in D]) > 0:
                ...
                print("WARNING: An entry in the '" + name +
                      "' column contains at least one instance of the "
                      "line terminator '" + lineterminator + "' and therefore will use "
                      "the Python csv module quoting convention (see online " 
                      "documentation for Python's csv module).  You may want "
                      "to choose another delimiter not appearing in records, " 
                      "for performance reasons.") 
    ...
    if UseComplex is True:
        csv.writer(F, dialect=dialect).writerows(([col[i] for col in ColStr] for i in range(len(ColStr[0]))))
    else:
...
def inferdialect(fname = None, datalines = None, delimiter_regex=None, verbosity=DEFAULT_VERBOSITY):
    """
    ...
    **Returns**
    ...
    if delimiter_regex:
        ...
        V = []
        for g in G:
            lvec = np.array([len(r) for r in  list(csv.reader(datalines[:1000],dialect=g))])  
            V += [lvec.var()]
        ...
        if len(G) > 0:
            ...
            if V.min() > 0:
                ...
                if fnamedelim not in delims:
                    fnamevar = np.array([len(r) for r in list(csv.reader(datalines[:1000],delimiter=fnamedelim))]).var()
                    if fnamevar < V.min():
...
def readstoredmetadata(fname, skiprows=0,linenumber=None, comments = '#', 
        ...
        if linenumber is None:
            if comments:
                ...
                if len(phlines) == 1:
                    ...
                    if verbosity >= 9:
                        print 'Searching for metametadata lines up to and including line', len(phlines) - 1, ', where comments end.'
            else:
                phlines = [fh.readline()]
                if verbosity >=9:
                    print 'No comments found, looking for metametadata on line 0.'
        else:
    ...
    if metametadata:
        ...
        Tval = max([v if isinstance(v,int) else max(v) for v in  metametadata.values()])
        fh = file(fname, 'rU')
...
def processmetadata(metadata,items = None, comments=None, 
      delimiter_regex=None,ncols = None, verbosity=DEFAULT_VERBOSITY):
...
def saveHSV(fname, X, printheaderfile=True):
    ...
    **See Also:**
    ...
    for i in range(len(keys)):
        if sum(pairwise[i]) == 0:
            Xi = X[keys[i]]
            inames = set(Xi.dtype.names)
            for k in Xi.coloring.keys():
                if set(Xi.coloring[k]) == inames:
                    Xi.coloring.pop(k)
            saveHSV(fname + keys[i] + '.hsv/', Xi, printheaderfile)
            names = [n for n in names if n not in inames]
...
def appendHSV(fname, RecObj, order=None):
    ...
    **See Also:**
    ...
    if hasattr(RecObj, 'coloring'):
        ...
        for i in range(len(keys)):
            if sum(pairwise[i]) == 0:
                RecObji = RecObj[keys[i]]
                inames = set(RecObji.dtype.names)
                for k in RecObji.coloring.keys():
                    if set(RecObji.coloring[k]) == inames:
                        RecObji.coloring.pop(k)            
                appendHSV(fname + keys[i] + '.hsv/', RecObji, order)
                names = [n for n in names if n not in inames]
